...
{
  var self = {routers:[], lines:{}, bridgeLine:{}, all:{}, links:[], rels:{}, raws:{}, paths:[], bridgeCache:{}, networks:{}, CSets:{}};
  ...
  // need a router to connect to, addRouter({ip:"1.2.3.4", port:5678, public:"PEM"})
  self.addRouter = addRouter;
...
function addRouter(arg) {
  var self = this;
  if(!arg.parts) return warn("invalid args to addSeed",arg);
  var router = self.whokey(arg.parts,false,arg.keys);
  if(!router) return warn("invalid router info",arg);
  if(Array.isArray(arg.paths)) arg.paths.forEach(function(path){
    path = router.pathGet(path);
    path.router = true;
  });
...
function online(callback)
{
  ...
  var dones = self.routers.length;
  if(!dones) {
    warn("no routers");
    return callback(null,0);
  }

  // safely callback only once or when all routers return
  function done()
  {
    if(!dones) return; // already called back
    var alive = self.routers.filter(function(router){return pathValid(router.to)}).length;
    if(alive)
    {
      callback(null,alive);
      dones = 0;
  ...
  self.routers.forEach(function(router){
    router.link(function(){
      if(pathValid(router.to)) router.pathSync();
      done();
    });
  });
}
...
function whois(hashname)
{
  ...
  {
    if(path.router) return false; // never remove a router-path
...
//    if((Math.floor(Math.random()*10) == 4)) return warn("testing dropping randomly!");
  ...
  hn.pathSync = function()
  {
    ...
    hn.raw("path",{js:js, timeout:10*1000}, function(err, packet){
      ...
      // if path answer is from a router, update our public ip/port in case we're behind a NAT
      if(packet.from.isRouter && typeof packet.js.path == "object" && packet.js.path.type == "ipv4" && !isLocalIP(packet.js.path.ip))
      {
...
function inRelay(chan, packet)
{
  ...
  // if the active relay is failing, try to create one via routers
